...
public class Solver {
    ...
    private Set<Invoke> staticInvokeSet;
    ...
    private class StmtProcessor implements StmtVisitor<Void> {
        ...
        public Void visit(Invoke stmt) {
            if (stmt.isStatic()) {
                ...
                // Process taint source
                if (taintAnalysis.isSource(m) && stmt.getLValue() != null) {
                    Var r = stmt.getLValue();
                    Obj taint = taintAnalysis.makeTaint(stmt, taintAnalysis.getSourceType(m));
                    CSObj csTaint = csManager.getCSObj(contextSelector.getEmptyContext(), taint);
                    CSVar rCSVar = csManager.getCSVar(context, r);
                    workList.addEntry(rCSVar, PointsToSetFactory.make(csTaint));
                }
                taintAnalysis.taintStaticPropagate(stmt, m, context);
    ...
    private void analyze() {
        while (!workList.isEmpty()) {
            ...
            // System.err.println(entry.pointer() + " -> " + entry.pointsToSet());
    ...
    private void processCall(CSVar recv, CSObj recvObj) {
        ...
        for (Invoke invoke : invokes) {
            ...
            // Process taint source
            if (taintAnalysis.isSource(m) && invoke.getLValue() != null) {
                Var r = invoke.getLValue();
                Obj taint = taintAnalysis.makeTaint(invoke, taintAnalysis.getSourceType(m));
                CSObj csTaint = csManager.getCSObj(contextSelector.getEmptyContext(), taint);
                CSVar rCSVar = csManager.getCSVar(recv.getContext(), r);
                workList.addEntry(rCSVar, PointsToSetFactory.make(csTaint));
            }

            // Process taint propagate
            taintAnalysis.taintPropagate(invoke, m, recv);
        }
        if (taintAnalysis.isTaint(recvObj.getObject())) {
            for (Var var1 : csManager.getVars()) {
                for (Invoke invoke1 : var1.getInvokes()) {
                    if (invoke1.getInvokeExp().getArgs().contains(recv.getVar())) {
                        System.out.println(invoke1);
                        CSVar csVar1 = csManager.getCSVar(recv.getContext(), var1);
                        for (CSObj csObj : csVar1.getPointsToSet().getObjects()) {
                            taintAnalysis.taintPropagate(invoke1, resolveCallee(csObj, invoke1), csVar1);
                        }
                    }
                }
            }
            for (Invoke staticInvoke : staticInvokeSet) {
                if (staticInvoke.getInvokeExp().getArgs().contains(recv.getVar())) {
                    taintAnalysis.taintStaticPropagate(staticInvoke, staticInvoke.getMethodRef().resolve(), recv.getContext());
                }
            }
        }
    }
